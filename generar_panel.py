...
export default function PanelProvincia({ provincia, datos, onCerrar }: Props) {
  ...
  // Filtrar datos: si es nacional excluye subprovinciales, si es provincia filtra por nombre
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm h-full">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          {esNacional && (
            <p className="text-xs text-slate-400 mt-0.5">Hacé clic en una provincia para ver el detalle</p>
          )}
        </div>
        {!esNacional && (
      ...
      <div className="space-y-3">
        {niveles.map(nivel => {
          const estatal = datosFiltrados.find(d => d.nivel === nivel && d.sector === "Estatal")?.total_alumnos || 0;
          const privado = datosFiltrados.find(d => d.nivel === nivel && d.sector === "Privado")?.total_alumnos || 0;
          const totalNivel = estatal + privado;
          const pct = totalNivel > 0 ? (privado / totalNivel) * 100 : 0;
          ...
          return (
            <div key={nivel} className="border border-slate-100 rounded-xl p-3">
              ...
              <div className="w-full bg-slate-100 rounded-full h-1.5">
                <div className="h-1.5 rounded-full" style={{ width: `${100 - pct}%`, background: "#1a3a6b" }} />
              </div>
...
print("PanelProvincia.tsx generado OK")
